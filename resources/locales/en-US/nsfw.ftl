...
upvote-msg = Please upvote me on Top.gg or DBL to use this command. { $emoji }
...
image-report-success = Here is the report link:
    { $link }
...
auto-setup-no-premium = You cannot have more than { $count } autoporn channels per server.
...
auto-setup-enable-no-webhooks-perm = I need the "Manage Webhooks" permission in order to be able to send autoporn messages in this channel!
...
auto-task-disabled = Set this channel to NSFW back and use { $autocmd_mention } command to re-enable autoporn in this channel.
...
auto-task-noperms-desc = Set the correct permissions and use the { $autocmd_mention } command to re-enable autoporn in this channel.
...
auto-task-maxwebhooks-desc = A Discord channel can have a maximum of 10 webhooks, and this channel has hit the maximum.
...
auto-cleanup-msg = Autoporn was disabled in this channel because this server has hit the maximum channel enabled (5).
