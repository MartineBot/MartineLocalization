...
streams-autorefresh = Aggiornamento automatico
...
posts-desc =
    ...
    - If you want to delete the alert you can use the same command, and click on the "Disable" button.
posts-message = Messaggio
# max.length: 80
posts-settings-msg-btn = Modifica messaggio
# max.length: 45
...
# max.length: 100
help-alerts_streams_kick = Gestisci l'avviso di una live per un utente di Kick.
# max.length: 100
alerts_streams_kick-parameter-user_name_or_url-description = Il nome o URL del canale di Kick.
# max.length: 100
