...
# $channel is the channel mention
sb-setup-already-exists = A starboard already exists in { $channel }.
...
# $emoji is the configured star emoji
sb-setup-tips = • React with { $emoji } on any message to star it
...
sb-reset-description = Are you sure you want to reset the starboard?
...
sb-embed-variables = `{"{"}stars{"}"}` - Star count
